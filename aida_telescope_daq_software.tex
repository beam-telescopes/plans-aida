...
\abstract{A high resolution ($\sigma \approx \SI{2}{\micro\meter}$)
...
detectors. 
  ...
  \item \eutel{} \cite{eutelsite}, a library that provides tools for the spatial
...
testbeams with the EUDET-family of beam telescopes since 2007.  It allows the
easy (but optional) integration of the device-under-test and its DAQ
into the telescope data stream and gives the user a central interface
to control and monitor the operation of the full system. EUDAQ
features a very modular design in which individual components
communicate via TCP/IP and can run on different networked machines.
