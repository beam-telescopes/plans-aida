...
processors. These collect packets (or optionally their meta-data only) by
accessing the files written by the data collectors and retrieving
specific trigger and clock ranges.
...
\item alternatively, the readers can find the packet(s) corresponding
  ...
  collector's file.
\item the reader processor will use file-system access to the data; a
  index file containing all the packets' meta information and seek
  position for the main binary file could speed up the time needed to
  retrieve the desired packet.
...
systems, allowing hardware-aided pre-selection of packets: As shown in
figure~\ref{fig:schematicpolling}, this can be performed in two
steps. First, a hardware signal from the TLU to all DAQs is issued on
request to allow very fast devices to cache packets for later
retrieval by the \emph{run monitor}. This TLU signal is timestamped
and included in the next packet to mark it. Then, the \emph{reader
  processors} are requested to send all packets containing such TLU
marker timestamp. If a DAQ does not include a timestamp for this
signal in its packets, either the run monitor or the \emph{reader}
determines the device's packet(s) that would correspond to the
time/trigger range containing the marker signal and retrieves it.

Implications:
\begin{itemize}
\item this system requires information from the TLU producer to be
  available if the device's DAQ itself does not react to the marker signal
\end{itemize}


At first, only the latter mechanism (purely run-monitor-driven request
for specific trigger number or clock range) should be implemented.
